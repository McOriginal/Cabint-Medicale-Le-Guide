...
import {
  useOneExterneOrdonnance,
  useUpdateExterneOrdonnance,
} from '../../Api/queriesExterneOrdonnance';

export default function UpdateExterneOrdonnance() {
  const param = useParams();
  const navigate = useNavigate();
  const { mutate: updateOrdonnance } = useUpdateExterneOrdonnance();
  const { data: selectedOrdonnance } = useOneExterneOrdonnance(param.id);

  const [ordonnanceItems, setOrdonnanceItems] = useState([]);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // useEffet pour initialiser la mise à jour
  useEffect(() => {
    if (selectedOrdonnance) {
      setOrdonnanceItems(
        selectedOrdonnance.items.map((item) => ({
  ...
  const validation = useFormik({
    ...
    initialValues: {
      traitement: selectedOrdonnance ? selectedOrdonnance.traitement : '',
      patient: selectedOrdonnance ? selectedOrdonnance.patient : '',
      doctor: selectedOrdonnance ? selectedOrdonnance.doctor : '',
      ordonnanceDate: selectedOrdonnance
        ? selectedOrdonnance.ordonnanceDate
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      updateOrdonnance(
        { id: selectedOrdonnance?._id, data: payload },
        {
          onSuccess: () => {
  ...
  return (
    <div className='page-content'>
      <Container fluid>
        <Breadcrumbs
          title='Ordonnance Externe'
          breadcrumbItem='Modifier Ordonnance'
        />
